...
// Revised version of aes_ctr.org.x: fixes the block-counter underflow on a
// zero-length command. In the original, a command with msg_bytes == 0 makes
// blocks_left wrap from 0 to 2^32 - 1 and locks the FSM in PROCESSING; the
// revision saturates the decrement so the proc returns to IDLE instead.
...
pub proc aes_ctr {
    ...
    next(state: State) {
        ...
        // Saturating decrement: a zero-length command must not wrap the
        // counter and lock the FSM in PROCESSING.
        let blocks_left = if blocks_left == u32:0 { u32:0 }
                          else { blocks_left - u32:1 };
