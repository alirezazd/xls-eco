...
int PickUnrollDim(const Schedule& sch, const CodegenConfig& cfg) {
  // Check if any dimensions have explicit unroll factors
...
mlir::LogicalResult GenerateUnrollPlan(const LinalgGeneric& g,
                                      ...
                                      UnrollPlan& plan) {
  // Use the global singleton config
  return GenerateUnrollPlan(g, A, GetLinalgCodegenConfig(), plan);
}

mlir::LogicalResult GenerateUnrollPlan(const LinalgGeneric& g,
                                      const BroadcastAnalysis& A,
                                      const LinalgCodegenConfig& cfg,
                                      UnrollPlan& plan) {
  plan.dims.clear();
  
  // Pick the dimension to unroll based on the linalg generic operation
  int unroll_dim = -1;
  for (const auto& [dim, factor] : cfg.unroll()) {
    if (factor > 1) {
      unroll_dim = dim;
  ...
  int unroll_factor = cfg.unroll_factor(unroll_dim);
